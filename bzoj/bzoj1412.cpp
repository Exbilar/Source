...
int que[maxm],dpt[maxm],ddf[maxn][maxn];
int cnt = 1,n,m,S,T;
...
int main(){
	scanf("%d%d",&n,&m);
	for(int i = 1;i <= n;i++)
		for(int j = 1;j <= m;j++) scanf("%d",&ddf[i][j]);
	...
	for(int i = 1;i <= n;i++){
		for(int j = 1;j <= m;j++){
			int pos = (i - 1) * m + j;
			if(ddf[i][j] == 1) ins(S,pos,INF);
			else if(ddf[i][j] == 2) ins(pos,T,INF);
			if(OK(i - 1,j)) ins(pos,pos - m,1);
			if(OK(i + 1,j)) ins(pos,pos + m,1);
			if(OK(i,j - 1)) ins(pos,pos - 1,1);
			if(OK(i,j + 1)) ins(pos,pos + 1,1);
		}
	}
